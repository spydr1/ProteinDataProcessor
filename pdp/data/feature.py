...
class AminoAcid(object):
    ...
    def __init__(self,aa):
        ...
        if isinstance(aa,str) :
            self.sequence = aa
            self._idx = self._aa_idx(aa) # todo : 아미노산은 사실 string 이면 되는데 .. 데이터 편의를 위해서 index를 추가 해놓는게 맞을까? 별로 크지 않아서 상관없지만 .. 그래도 낭비는 낭비일지도 ?
    ...
    def _vocab_check(self,sequence) -> None:
        ...
        for i, current_seq in enumerate(sequence) :
            if current_seq not in vocab.aa_idx_vocab :
                raise ValueError(f"\"{current_seq}\" is not existed in vocab. idx number : {i}")
    ...
    def _idx_aa(self, idx: List[int]) -> List[str]:
        """
    ...
    def vocab_check(self) -> None:
        for i, current_ss3 in enumerate(self.ss3) :
            if current_ss3 not in vocab.ss3_idx_vocab :
                raise ValueError(f"\"{current_ss3}\" is not existed in vocab. idx number : {i}")
    ...
    def vocab_check(self) -> None:
        for i, current_ss8 in enumerate(self.ss8) :
            if current_ss8 not in vocab.ss8_idx_vocab :
                raise ValueError(f"\"{current_ss8}\" is not existed in vocab. idx number : {i}")
